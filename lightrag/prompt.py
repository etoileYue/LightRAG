...
PROMPTS["entity_extraction"] = """-目标-
  ...
  给定一份与活动相关的文本和一个实体类型列表，从文本中识别所有属于这些类型的实体，并确定所识别实体之间的所有关系，直接输出结果。
...
-真实数据-
  ...
  请根据上述步骤和示例，识别并格式化实体及其关系，直接输出结果。
...
] = """你将作为一个助手，负责生成以下数据的全面摘要，提供一个结构清晰、内容全面的描述，涵盖所有相关信息。直接输出结果。
### 任务要求
...
] = """在最近的一次提取中，很多实体信息丢失了，现在在下面以相同的格式添加这些丢失的信息，直接输出结果:
"""
